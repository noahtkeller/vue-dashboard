<template>
  <v-progress-circular v-show="!fileName" :size="100" indeterminate color="primary"></v-progress-circular>
  <MovieClient v-model="fileName" class="movie-client" :infoHash="infoHash"/>
</template>

<script>
import { MovieClient } from '@noahtkeller/vue-yify-client';
  ...
  computed: {
    ...
    torrent: {
      get() {
        ...
        return this.$store.state._movies[this.$route.params.id]?.torrents.findIndex(({ quality }) => quality === '720p');
      }
    },
    infoHash: {
      get() {
