<template>
  <v-hover v-slot="{ isHovering, props }">
    <v-card
      ...
      @click="$emit('click:details', movie.id)"
      outlined
      v-bind="props"
    >
      <v-card-title>
      ...
      <v-img cover :src="movie.large_cover_image" aspect-ratio=".66">
        ...
        <v-fade-transition>
          <div v-if="isHovering" class="btn-container">
            ...
            <h2 class="mdc-theme--on-primary rating">
              <t-string :t="rating" :count="movie.rating" />
            </h2>
            <div>
              <v-btn
                @click.stop="$emit('click:watch', movie.id)"
                class="mdc-theme--secondary-bg mdc-theme--on-secondary"
                >Watch&nbsp;<v-icon>mdi-play</v-icon></v-btn
              >
            </div>
            <h2 class="mdc-theme--on-primary rating">
...
<script>
import { rating } from '@noahtkeller/common-strings/i18n/keys/yify';

export default {
  name: 'SearchResultItem',
  props: ['movie'],
  emits: ['click:details', 'click:watch'],
  data() {
    return { overlay: false, rating };
  },
};
</script>
